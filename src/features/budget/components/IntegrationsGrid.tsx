import { erpIntegrations, type ErpIntegration } from "@/features/budget/data";
...
function isInitiallyConnected(i: ErpIntegration) {
...
export function IntegrationsGrid() {
  ...
  const [lastSyncByNome, setLastSyncByNome] = useState<Record<string, string>>({});
  const [configTarget, setConfigTarget] = useState<ErpIntegration | null>(null);
  const [endpoint, setEndpoint] = useState("");

  const handleConnect = (nome: string) => {
    toast.promise(
      new Promise<void>((resolve) => {
        setTimeout(() => {
          setConnectedNames((prev) => new Set(prev).add(nome));
  ...
  const openConfig = (i: ErpIntegration) => {
    setConfigTarget(i);
