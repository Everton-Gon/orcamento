import { costCenters, brl, type CostCenter } from "@/features/budget/data";
...
type SortKey = keyof CostCenter | "utilizacao";

function matchesSearch(row: CostCenter, q?: string) {
...
function matchesDepartments(row: CostCenter, allowed: string[] | null) {
...
export function CostCentersTable({
  ...
  data?: CostCenter[];
  /** Texto da pesquisa global (ex.: query string na página Centros). */
  searchText?: string;
}) {
  ...
  const [draftAllowed, setDraftAllowed] = useState<Record<string, boolean>>({});
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {searchText ? (
              <span className="block mt-1 text-foreground/80">
                Pesquisa ativa: &quot;{searchText}&quot; · {sortedRows.length} resultado(s)
              </span>
            ) : null}
